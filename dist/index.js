...
function updateCounter(element, shouldIncrement) {
    ...
    if (newValue === 0)
        updateActiveState(element, false);
}
...
function handleAddSubmit(event) {
    ...
    if (displayErrors(mainForm, errors))
        return;
...
function getContactParams(element) {
    ...
    const [jsonAttribute] = Object.values(Object.assign({}, targetContact.dataset));
    ...
    function resetActiveItems() {
        activeItems.forEach(elem => {
            ...
            if (!elem.nextElementSibling.hidden)
                toggleVisibility(elem.nextElementSibling);
        });
    }
    clearAllContacts();
    resetActiveItems();
...
function handleFoundContactClick(event) {
    ...
    if (editIcon && editIcon.contains(event.target)) {
        const { openEditModal, handleCloseClick, handleModalBackdropClick, closeEditModal } = initEditModal(editModal, editModalForm, closeIcon, handleEditSubmit);
        openEditModal();
        ...
        function handleEditSubmit(event) {
            ...
            if (displayErrors(editModalForm, errors))
                return;
            ...
            if (currentIdentifier !== newIdentifier)
                moveContact(currentIdentifier, currentListItem, newIdentifier);
            const searchInputValue = document.querySelector('.js-search-modal__input').value;
            if (searchInputValue)
                refreshContactList(currentIdentifier, searchedContacts);
            updateEmptyState();
            closeEditModal(handleCloseClick, handleModalBackdropClick);
        }
    }
...
function displayErrors(formElement, errors) {
    ...
    Object.entries(errors).forEach(([fieldName, fieldErrors]) => {
        if ((fieldErrors === null || fieldErrors === void 0 ? void 0 : fieldErrors.length) && fieldName !== 'general') {
